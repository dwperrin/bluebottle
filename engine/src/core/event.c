...
b8 event_initialise() {
    ...
    bb_zero_memory(&state, sizeof(state));
...
void event_shutdown() {
    // Free the events arrarys. And objects pointed to should be destroyed on their own.
...
b8 event_register(u16 code, void* listener, PFN_on_event on_event) {
    ...
    u64 registed_count = darray_length(state.registered[code].events);
    ...
    // If at this point, no duplicated was found. Proceed with registration.
