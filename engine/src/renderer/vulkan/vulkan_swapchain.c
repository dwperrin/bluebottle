...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        BBFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
...
void vulkan_swapchain_present(
    ...
    // Return the image to the swapcaind for presentation.
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        BBFATAL("Failed to present swap chain image!");
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    // Clamp to the vlaue allowed by the GPU.
    ...
    swapchain_extent.width = BBCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = BBCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if(!swapchain->images)
    {
        swapchain->images = (VkImage*)bb_allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if(!swapchain->views) {
        swapchain->views = (VkImageView*)bb_allocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        BBFATAL("Failed to find a supported format!");
    }
    ...
    BBINFO("Swapchain created successfully.");
}
