...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    BBINFO("Creating logical device...");
    ...
    BBINFO("Graphics command pool created.");
...
void vulkan_device_destroy(vulkan_context* context){
    ...
    BBINFO("Destroying command pools...");
    ...
    BBINFO("Destroying logical device...");
    if (context->device.logical_device) {
        vkDestroyDevice(context->device.logical_device, context->allocator);
        context->device.logical_device = 0;
    }

    // Physical decvices are not detroyed
    BBINFO("Releasing physical device resources ...");
...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats = bb_allocate(sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
            physical_device,
            surface,
    ...
    if (out_support_info->present_mode_count != 0) {
        if(!out_support_info->present_modes) {
            out_support_info->present_modes = bb_allocate(sizeof(VkPresentModeKHR) * out_support_info->present_mode_count, MEMORY_TAG_RENDERER);
        }
        ...
        if (result) {
            ...
            for (u32 j = 0; j < memory.memoryHeapCount; ++j) {
                ...
                if (memory.memoryHeaps[j].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
                    BBINFO("Local GPU memory: %.2f GiB", memory_size_gib);
                } else {
                    BBINFO("Shared System memory: %.2f GiB", memory_size_gib);
                }
            }
    ...
    if (
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                bb_free(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                bb_free(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, MEMORY_TAG_RENDERER);
            }
            BBINFO("Required swapchain support not present, skipping device.");
            return FALSE;
        }
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = bb_allocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extension_count,
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        BBINFO("Required extension not found: '%s', skipping device", requirements->device_extension_names[i]);
                        bb_free(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
                        return FALSE;
                    }
                }
            }
        ...
        if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            BBINFO("Device does not support samplerAnisotropy, skipping.");
            return FALSE;
        }
