...
void vulkan_framebuffer_create(
    ...
    // Take a copy of the attachments, renderpass and attachmnet count
    out_framebuffer->attachments = bb_allocate(sizeof(VkImageView) * attachment_count, MEMORY_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context* context, vulkan_framebuffer* framebuffer) {
    ...
    if (framebuffer->attachments) {
        bb_free(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMORY_TAG_RENDERER);
